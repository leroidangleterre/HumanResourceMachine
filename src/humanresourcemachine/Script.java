...
public class Script extends MyDefaultComponent implements Observer {
    ...
    public void initWorkers(ArrayList<Worker> list) {
        System.out.println("Script.initWorkers, before: " + this.getNbWorkers());
        ((ScriptModel) model).removeAllWorkers();
        for (Worker w : list) {
            ((ScriptModel) model).addWorker(w);
        }
        System.out.println("Script.initWorkers, after: " + this.getNbWorkers());
    }

    private int getNbWorkers() {
        int count = 0;
        for (Instruction ins : instList) {
            count += ins.getNbWorkers();
        }
        return count;
    }
...
//        System.out.println("Clearing script");
    ...
    /**
     * Run the script as quickly as possible until the end.
     * WARNING: loops endlessly if the script does not terminates.
     */
    public void playAll() {
        while (hasActiveWorkers()) {
            step();
        }
        System.out.println("End of playAll. ActiveWorkers: " + hasActiveWorkers());
    }
    ...
    /**
     * Check if at least one worker is still active.
     *
     * @return true when at least one worker is active, false otherwise (i.e.
     * all workers have finished their jobs).
     */
    private boolean hasActiveWorkers() {

        for (Instruction inst : instList) {
            if (inst.hasActiveWorkers()) {
                System.out.println("inst " + inst + " has active workers: " + inst.getNbWorkers());
                return true;
            }
        }
        return false;
